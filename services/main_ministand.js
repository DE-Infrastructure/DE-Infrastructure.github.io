...
// какие сервисы считаем мини-стендом (по имени, регистр не важен)
const MINISTAND_NAMES = [
  'airflow',
  'gitea',
  'postgres',
  'postgresql'
...
async function loadServices() {
  try {
    ...
    // фильтруем только нужные сервисы
    services = services.filter(s => {
      const name = (s.name || '').toLowerCase();
      return MINISTAND_NAMES.some(key => name.includes(key));
    });

    allServices = services;

    // теги только по этим сервисам
